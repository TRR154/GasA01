...
{
   if( abs(pb - db) <= min(abstol, reltol) )
      return 1;
   else
      return 0;
}
...
BEGIN {
   ...
   checksol = 0;                # should the solution check of SCIP be parsed and counted as a fail if best solution is not feasible?
   ...
   # statistics for gap and primal availability
    total_gap_sum = 0.0;
    gap_count = 0;
    no_primal = 0;
    totaltimeADM = 0;
...
/^@01/ {
   ...
   # initialize ADM heuristic stats for this instance
   adm_exectime = 0.0;
   adm_best     = 0;


   # Escape _ for TeX and start with "nomination_"
   n = split(prob, a, "_");
   pprob = a[1];
   startindex = 2;
   if ( a[1] == "nomination" && n > 1 )
   {
      pprob = a[2];
      startindex = 3;
   }
...
/^[ \t]*ADM[ \t]*:/ {
    adm_exectime = ($3 + 0);
    adm_best     = ($7 + 0);
    adm_besttotal += adm_best;
    totaltimeADM += adm_exectime;
    callsADM +=1;
}
...
/^=ready=/ || /==MISSING==/ {
   ...
   if( TEXFILE != "" )
   {
         ...
         printf("\\usepackage{supertabular,longtable}\n")                           >TEXFILE;
      ...
      if (totaltimeADM > 0)
      {
         ...
         #printf("\\begin{table}[tb]\n")                                  >TEXFILE;                       
         printf("\\centering\n")                                          >TEXFILE;
         printf("\\begin{longtable}{@{}l@{\\quad}r@{\\quad}r@{\\quad}r@{\\quad}r@{\\quad}r@{\\quad}r@{\\quad}r@{}}\n") >TEXFILE;
         printf("\\caption{Results for settings %s}\n",settings)          >TEXFILE; 
         printf("\\label{tab:results-%s}\\\\ \n",settings)                     >TEXFILE;
         printf("\\toprule\n")                                            >TEXFILE;
         printf("Name & Dual Bound &  Primal Bound & Gap\\%% & Time & Time ADM & Nodes & ADM best sol \\\\\n") >TEXFILE;
         printf("\\midrule\n")                                            >TEXFILE;
      }
      else
      {
         ...
         #printf("\\begin{table}[tb]\n")                                   >TEXFILE;
         printf("\\centering\n")                                          >TEXFILE;
         printf("\\begin{longtable}{@{}l@{\\quad}r@{\\quad}r@{\\quad}r@{\\quad}r@{\\quad}r@{}}\n") >TEXFILE;
         printf("\\caption{Results for settings %s}\n",settings)          >TEXFILE; 
         printf("\\label{tab:results-%s}\\\\ \n",settings)                     >TEXFILE;
         printf("\\toprule\n")                                            >TEXFILE;
         printf("Name & Dual Bound &  Primal Bound & Gap\\%% & Time & Nodes \\\\\n") >TEXFILE;
         printf("\\midrule\n")                                            >TEXFILE;
      }
   }
      ...
      if( totaltimeADM > 0)
      {
         ...
         tablehead2 = tablehead2"| Type | Conss |  Vars | Conss |  Vars |   Dual Bound   |  Primal Bound  | Gap%% |  Iters  |  Nodes |  Time | t ADM | ADMsol| Status ";
         tablehead3 = tablehead3"+------+-------+-------+-------+-------+----------------+----------------+------+---------+--------+-------+-------+-------+--------";
      }
      else if(totaltimeADM < -1) # summarized table. 
      {
         tablehead1 = tablehead1"-+--------+----------+---------+--------------+--------------+--------";
         tablehead2 = tablehead2" |  Gap%%  |  Nodes   |   Time  |   Time ADM   | ADM best sol | Status ";
         tablehead3 = tablehead3"-+--------+----------+---------+--------------+--------------+--------";
      }
      else
      {
         tablehead1 = tablehead1"+------+--- Original --+-- Presolved --+----------------+----------------+------+---------+--------+-------+--------";
         tablehead2 = tablehead2"| Type | Conss |  Vars | Conss |  Vars |   Dual Bound   |  Primal Bound  | Gap%% |  Iters  |  Nodes |  Time | Status ";
         tablehead3 = tablehead3"+------+-------+-------+-------+-------+----------------+----------------+------+---------+--------+-------+--------";
      }
       ...
       (!onlyintestfile || intestfile[prob]) ) #&& conf_prop + conf_infLP + conf_bndEx + conf_pseud + conf_strbr > 0 )
   {
      ...
      if( gap < 0.0 )
         ...
         gapstr = " Large";
        # accumulate gap statistics if defined
        if (gap >= 0.0) { total_gap_sum += gap; gap_count++; }
        # count cases without a primal solution (primal bound infinite)
        if (abs(pb) >= infty) no_primal++;
      ...
      if( !onlypresolvereductions || origcons > cons || origvars > vars )
      {
         if (TEXFILE != "")
         {
            ...
            if( totaltimeADM > 0 )
            {
               printf("%-*s & %6s & %16.9g & %16.9g & %8.1f & %7.1f & %7d & %7d \\\\\n", namelength, pprob, db, pb, gapstr, tottime,  adm_exectime, bbnodes, adm_best) > TEXFILE;
            }
            else
            {
               printf("%-*s & %6s & %16.9g & %16.9g & %8.1f & %7d \\\\\n", namelength, pprob, db, pb, gapstr, tottime, bbnodes) > TEXFILE;
            }
   ...
   if( totaltimeADM > 0 )
   {
      printf("%-*s  %-5s %7d %7d %7d %7d %16.9g %16.9g %6s %9d %8d %7.1f %7.1f %7d ", namelength, shortprob, probtype, origcons, origvars, cons, vars, db, pb, gapstr, simpiters, bbnodes, tottime, adm_exectime, adm_best);
   }
   else
   {
      printf("%-*s  %-5s %7d %7d %7d %7d %16.9g %16.9g %6s %9d %8d %7.1f ", namelength, shortprob, probtype, origcons, origvars, cons, vars, db, pb, gapstr, simpiters, bbnodes, tottime);
   }
...
END {
   ...
   # Additional aggregated statistics (average gap and no primal solutions)
    avg_gap = (gap_count > 0) ? total_gap_sum / gap_count : 0.0;
    if (gap_count > 0) {
        printf("Average gap over %d instances: %6.2f\n", gap_count, avg_gap);
    } else {
        printf("Average gap over %d instances: %s\n", gap_count, "N/A");
    }
    printf("Solved instances : %i\n", pass);
    printf("Instances with no primal solution: %d\n", no_primal);
    if( totaltimeADM > 0 )
      {
         avgTimeADM = (callsADM > 0) ? totaltimeADM / callsADM : 0.0;
         printf("Average time spent in ADM: %6.2f\n", avgTimeADM);
      }
    avgTotalTime = stottime / nprobs;
    printf("Average time over all instances: %6.2f\n", stottime / nprobs);  


    # also write these stats to TEXFILE if requested
    if (TEXFILE != "") 
    { 
        if( totaltimeADM > 0 )
         {
            printf("\\midrule\n")  >TEXFILE;
            printf("%-14s %2d & -- & -- &  %6.1f & %7.1f & %7.1f & %7.1f & %7d \\\\\n" , "Avg. of", nprobs, avg_gap, avgTotalTime, avgTimeADM, nodegeom, adm_besttotal) >TEXFILE;
            printf("No primal sol: %d \\\\\n", no_primal) > TEXFILE;
            printf("Solved instances : %i \\\\\n", pass)>TEXFILE;
         }
         else
         {
            printf("\\midrule\n")  >TEXFILE;
            printf("%-14s %2d & -- & -- & %6.1f & %7.1f & %7.1f \\\\\n" , "Avg. of", nprobs, avg_gap, avgTotalTime, nodegeom) >TEXFILE;
            printf("\No primal sol: %d \\\\\n", no_primal) > TEXFILE;
            printf("Solved instances : %i \\\\\n", pass)>TEXFILE;
         }
    }
...
if( TEXFILE != "" )
{
   if( totaltimeADM > 0 )
   {
      # print header of tex file table WITH ADM columns
      printf("\\begin{table}[tb]\n")                                   >TEXFILE;
      printf("\\centering\n")                                          >TEXFILE;
      printf("\\caption{Results for settings %s}\n",settings)          >TEXFILE; 
      printf("\\label{tab:results:%s}\n", settings)                    >TEXFILE;
      printf("\\begin{tabular}{@{}l@{\\quad}r@{\\quad}r@{\\quad}r@{\\quad}r@{\\quad}r@{}}\n") >TEXFILE;
      printf("\\toprule\n")                                            >TEXFILE;
      printf("Averages for: & Gap \\%% & Time & Time ADM & Nodes & ADM best sol \\\\\n") >TEXFILE;
      printf("\\midrule\n")                                            >TEXFILE;
      printf("%-14s %2d & %6.1f & %7.1f & %7.1f & %7.1f & %7d \\\\\n" , "Avg. of", nprobs, avg_gap, avgTotalTime, avgTimeADM, nodegeom, adm_besttotal) >TEXFILE;
      printf("\\midrule\n")  >TEXFILE;
      printf("Number of solved instances : %i \\\\\n", pass)>TEXFILE;
      printf("\\midrule\n")  >TEXFILE;
      printf("Number of instances with no primal solution: %d \\\\\n", no_primal) > TEXFILE;
      printf("\\bottomrule\n")     >TEXFILE;
      printf("\\end{tabular}\n")  >TEXFILE;
      printf("\\end{table}\n\n")    >TEXFILE;
   }
   ...
   {
      # print header of tex file table WITHOUT ADM columns
      printf("\\begin{table}[tb]\n")                                   >TEXFILE;
      printf("\\centering\n")                                          >TEXFILE;
      printf("\\caption{Results for settings %s}\n",settings)          >TEXFILE; 
      printf("\\label{tab:results:%s}\n", settings)                    >TEXFILE;
      printf("\\begin{tabular}{@{}l@{\\quad}r@{\\quad}r@{\\quad}r@{}}\n") >TEXFILE;
      printf("\\toprule\n")                                            >TEXFILE;
      printf("Averages for: & Gap \\%% & Time & Nodes \\\\\n")          >TEXFILE;
      printf("\\midrule\n")                                            >TEXFILE;
      printf("%-14s %2d & %6.1f & %7.1f & %7.1f \\\\\n" , "Avg. of", nprobs, avg_gap, avgTotalTime, nodegeom) >TEXFILE;
      printf("\\midrule\n")  >TEXFILE;
      printf("Number of solved instances : %i \\\\\n", pass)>TEXFILE;
