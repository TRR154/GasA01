...
do
  ...
  then
      ...
      for i in `cat $DIR/$EVALFILE.eval` DONE
        ...
        then
            ...
            echo Missing $FILE --
				MISSINGFILES=$MISSINGFILES"Missing $FILE --\n"
  ...
  then
      ...
      # detect test used lpsolver
      LPSOLVER=`echo $EVALFILE | sed 's/check.\([a-zA-Z0-9_-]*\).gasa01.linux.x86_64.gnu.\([a-zA-Z0-9_]*\).\([a-zA-Z0-9_]*\).*/\3/g'`

      # detect test used settings
      SETTINGS=`echo $EVALFILE | sed 's/check.\([a-zA-Z0-9_-]*\).gasa01.linux.x86_64.gnu.\([a-zA-Z0-9_]*\).\([a-zA-Z0-9_]*\).moskito.\([a-zA-Z0-9_]*\).*/\4/g'`

      echo "Testset " $TSTNAME
      echo "Solver  " $SOLVER
      echo "Testset   " $TSTNAME > $RESFILE
      echo "Solver    " $SOLVER >> $RESFILE
      echo "LP-Solver " $LPSOLVER >> $RESFILE
      echo "Settings  " $SETTINGS >> $RESFILE
      echo -e $MISSINGFILES  >> $RESFILE
      ...
      # we should not check for SOLVER = scip here, because check.awk needs also to be called for examples with other names
          ...
          awk -f $CHECKWD/check.awk -v "TEXFILE=$TEXFILE" -v "PAVFILE=$PAVFILE" -v "ERRFILE=$ERRFILE" $AWKARGS $TESTFILE $SOLUFILE $OUTFILE | tee -a $RESFILE
      fi
