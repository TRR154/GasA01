/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
/*                                                                           */
/*    This file is part of the code gasa01, which contains code              */
/*    for Project A01                                                        */
/*    "Global Methods for Stationary and Instationary Gastransport"          */
/*    of the SFB/TRR 154 "Mathematical Modelling, Simulation and             */
/*    Optimization on the Example of Gas Networks".                          */
/*                                                                           */
/*    Copyright (C) 2015-     Discrete Optimization Group, TU Darmstadt      */
/*                                                                           */
/*    Based on SCIP  --- Solving Constraint Integer Programs                 */
/*                                                                           */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

/**@file   heur_lpround.c
 * @ingroup PRIMALHEURISTICS
 * @brief  rounding heuristic for fixing flow directions
 * @author Marc Pfetsch
 */

/*---+----1----+----2----+----3----+----4----+----5----+----6----+----7----+----8----+----9----+----0----+----1----+----2*/

#include "heur_lpround.h"
#include "scip/scip.h"
#include <strings.h>

#include "elements_gas.h"

/* default values for standard parameters that every primal heuristic has in SCIP */
#define HEUR_NAME             "lpround"
#define HEUR_DESC             "rounding flow directions"
#define HEUR_DISPCHAR         '~'
#define HEUR_PRIORITY         -1100000
#define HEUR_FREQ             -1
#define HEUR_FREQOFS          0
#define HEUR_MAXDEPTH         -1
#define HEUR_TIMING           SCIP_HEURTIMING_AFTERLPNODE
#define HEUR_USESSUBSCIP      TRUE      /**< does the heuristic use a secondary SCIP instance? */

#define DEFAULT_NODELIMIT     1LL       /**< maximal number of nodes to use in sub-SCIP */
#define DEFAULT_TRANSFERSOLS  TRUE      /**< Transfer solutions of main SCIP to sub-SCIP? */


/*
 * Data structures
 */

/** primal heuristic data */
struct SCIP_HeurData
{
   SCIP_Longint          nodelimit;          /**< maximal number of nodes to use in sub-SCIP */
   SCIP_Bool             transfersols;       /**< transfer solutions of main SCIP to sub-SCIP */
};


/*
 * Callback methods of primal heuristic
 */

/** copy method for primal heuristic plugins (called when SCIP copies plugins) */
static
SCIP_DECL_HEURCOPY(heurCopyLPRound)
{  /*lint --e{715}*/
   assert( scip != NULL );
   assert( heur != NULL );
   assert( strcmp(SCIPheurGetName(heur), HEUR_NAME) == 0 );

   /* call inclusion method of primal heuristic */
   SCIP_CALL( SCIPincludeHeurLPRound(scip) );

   return SCIP_OKAY;
}

/** destructor of primal heuristic to free user data (called when SCIP is exiting) */
static
SCIP_DECL_HEURFREE(heurFreeLPRound)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;

   assert( heur != NULL );
   assert( scip != NULL );

   /* get heuristic data */
   heurdata = SCIPheurGetData(heur);
   assert( heurdata != NULL );

   /* free heuristic data */
   SCIPfreeBlockMemory(scip, &heurdata);
   SCIPheurSetData(heur, NULL);

   return SCIP_OKAY;
}

/** execution method of primal heuristic */
static
SCIP_DECL_HEUREXEC(heurExecLPRound)
{  /*lint --e{715}*/
   SCIP_PROBDATA* probdata;
   SCIP_HEURDATA* heurdata;
   SCIP_HASHMAP* varmapfw;
   SCIP_RETCODE retcode;
   SCIP_Real timelimit;
   SCIP_Bool success;
   SCIP_VAR** subvars;
   SCIP_VAR** vars;
   SCIP* subscip;
   int nvars;
   SCIP_SOL* subsol;
   GAS_Network* network;
   int i;
   int j;

   assert( scip != NULL );
   assert( result != NULL );

   *result = SCIP_DIDNOTRUN;

   /* do not run recursively */
   if ( SCIPgetSubscipDepth(scip) > 0 )
      return SCIP_OKAY;

   /* only call heuristic, if an optimal LP solution is at hand or if relaxation solution is available */
   if ( SCIPgetLPSolstat(scip) != SCIP_LPSOLSTAT_OPTIMAL )
      return SCIP_OKAY;

   /* check whether there is enough time and memory left */
   SCIP_CALL( SCIPcheckCopyLimits(scip, &success) );

   if ( ! success )
      return SCIP_OKAY;

#ifdef SCIP_DEBUG
   SCIPverbMessage(scip, SCIP_VERBLEVEL_NORMAL, NULL, "running heuristic <%s> ...\n", SCIPheurGetName(heur));
   SCIPdebugMsg(scip, "Running <%s> ...\n", SCIPheurGetName(heur));
   *result = SCIP_DIDNOTFIND;
#endif

   /* get data */
   heurdata = SCIPheurGetData(heur);
   assert( heurdata != NULL );

   /* initialize the subproblem */
   SCIP_CALL( SCIPcreate(&subscip) );

   /* get variables */
   nvars = SCIPgetNOrigVars(scip);
   vars = SCIPgetOrigVars(scip);

   /* create the variable mapping hash map */
   SCIP_CALL( SCIPhashmapCreate(&varmapfw, SCIPblkmem(subscip), nvars) );

   /* mark that we copy the original (this is a work-around) */
   probdata = SCIPgetProbData(scip);
   assert( probdata != NULL );

   /* create copy of original problem - we use the original in order to avoid aggregations which would confuse our deletion below */
   SCIP_CALL( SCIPcopyOrig(scip, subscip, varmapfw, NULL, "heur_lpround", FALSE, FALSE, TRUE, &success) );

   if ( ! success )
   {
      SCIPerrorMessage("Was not able to copy SCIP during <%s>.\n", SCIPheurGetName(heur));

      SCIPhashmapFree(&varmapfw);
      SCIP_CALL( SCIPfree(&subscip) );

      return SCIP_ERROR;
   }

   /* copy subproblem variables into the same order as the source SCIP variables */
   SCIP_CALL( SCIPallocBufferArray(scip, &subvars, nvars) );
   for (i = 0; i < nvars; ++i)
      subvars[i] = (SCIP_VAR*) SCIPhashmapGetImage(varmapfw, vars[i]);

   /* transfer bounds */
   for (i = 0; i < nvars; ++i)
   {
      SCIP_Real lb;
      SCIP_Real ub;

      assert( subvars[i] != NULL );

      lb = SCIPvarGetLbLocal(vars[i]);
      ub = SCIPvarGetUbLocal(vars[i]);

      SCIP_CALL( SCIPchgVarLb(subscip, subvars[i], lb) );
      SCIP_CALL( SCIPchgVarUb(subscip, subvars[i], ub) );
   }

   /*  fix binVar - try to set flow direction values */
   network = probdata->network;
   for (j = 0; j < network->numarcs; ++j)
   {
      GAS_Arc* arc;
      SCIP_Real flowval;

      arc = &(network->arcs_ptr[j]);
      if ( arc->flowvar != NULL )
      {
         flowval = SCIPgetSolVal(scip, NULL, arc->flowvar); /* get LP val */

         if ( arc->positiveFlowBinvar != NULL && arc->negativeFlowBinvar != NULL )
         {
            SCIP_VAR* posBinvar;
            SCIP_VAR* negBinvar;
            SCIP_VAR* posFlowBinvar;
            SCIP_VAR* negFlowBinvar;

            posFlowBinvar = arc->positiveFlowBinvar;
            negFlowBinvar = arc->negativeFlowBinvar;

            assert( SCIPvarIsBinary(posFlowBinvar) );
            assert( SCIPvarIsBinary(negFlowBinvar) );

            posBinvar = (SCIP_VAR*) SCIPhashmapGetImage(varmapfw, posFlowBinvar);
            negBinvar = (SCIP_VAR*) SCIPhashmapGetImage(varmapfw, negFlowBinvar);

            assert( SCIPvarIsBinary(posBinvar) );
            assert( SCIPvarIsBinary(negBinvar) );

            /* change bounds if posBinvar not already fixed */
            if ( SCIPvarGetLbLocal(posBinvar) < 0.5 && SCIPvarGetUbLocal(posBinvar) > 0.5 )
            {
               if ( flowval > 0.0 )
               {
                  SCIP_CALL( SCIPchgVarLb(subscip, posBinvar, 1.0) );
                  SCIPdebugMsg(scip, "posBinvar %s fixed to 1 \n", arc->id);
               }
               else
               {
                  SCIP_CALL( SCIPchgVarUb(subscip, posBinvar, 0.0) );
                  SCIPdebugMsg(scip, "posBinvar %s fixed to 0 \n", arc->id);
               }
            }

            if ( SCIPvarGetLbLocal(negBinvar) < 0.5 && SCIPvarGetUbLocal(negBinvar) > 0.5 )
            {
               if ( flowval < 0.0 )
               {
                  SCIP_CALL( SCIPchgVarLb(subscip, negBinvar, 1.0) );
                  SCIPdebugMsg(scip, "negBinvar %s fixed to 1 \n", arc->id);
               }
               else
               {
                  SCIP_CALL( SCIPchgVarUb(subscip, negBinvar, 0.0) );
                  SCIPdebugMsg(scip, "negBinvar %s fixed to 0 \n", arc->id);
               }
            }
         }
      }
   }

   /* free hash map */
   SCIPhashmapFree(&varmapfw);

   /* transfer solutions */
   if ( heurdata->transfersols )
   {
      SCIP_SOL** sols;
      SCIP_SOL* sol;
      int nsols;

      nsols = SCIPgetNSols(scip);
      sols = SCIPgetSols(scip);
      for (i = 0; i < nsols; ++i)
      {
         SCIP_CALL( SCIPcreateSol(subscip, &subsol, NULL) );
         sol = sols[i];

         for (j = 0; j < nvars; ++j)
         {
            SCIP_Real val;

            val = SCIPgetSolVal(scip, sol, vars[j]);
            SCIP_CALL( SCIPsetSolVal(subscip, subsol, subvars[j], val) );
         }
         SCIP_CALL( SCIPaddSolFree(subscip, &subsol, &success) );
      }
   }

#ifdef SCIP_MORE_DEBUG
   /* for debugging, enable full output */
   SCIP_CALL( SCIPsetIntParam(subscip, "display/verblevel", 5) );
   SCIP_CALL( SCIPsetIntParam(subscip, "display/freq", 10000) );
#else
   /* disable statistic timing inside sub SCIP and output to console */
   SCIP_CALL( SCIPsetIntParam(subscip, "display/verblevel", 0) );
   SCIP_CALL( SCIPsetBoolParam(subscip, "timing/statistictiming", FALSE) );
#endif

   /* copy time and memory limits */
   SCIP_CALL( SCIPcopyLimits(scip, subscip) );

   SCIP_CALL( SCIPgetRealParam(subscip, "limits/time", &timelimit) );
   if ( timelimit > 600.0 )
   {
      SCIP_CALL( SCIPsetRealParam(subscip, "limits/time", 600.0) );
   }

   /* do not abort subproblem on CTRL-C */
   SCIP_CALL( SCIPsetBoolParam(subscip, "misc/catchctrlc", FALSE) );

   /* set strong node limit */
   SCIP_CALL( SCIPsetLongintParam(subscip, "limits/nodes", heurdata->nodelimit) );

#ifdef SCIP_MORE_DEBUG
   SCIP_CALL( SCIPwriteOrigProblem(subscip, "heurlpround.cip", "cip", FALSE) );

   SCIP_CALL( SCIPpresolve(subscip) );
   SCIP_CALL( SCIPwriteTransProblem(subscip, "heurlproundtrans.cip", "cip", FALSE) );
#endif

   /* solve subscip */
   retcode = SCIPsolve(subscip);

   if ( retcode != SCIP_OKAY )
   {
      SCIPwarningMessage(scip, "Error while solving subproblem in <%s>; sub-SCIP terminated with code <%d>.\n", SCIPheurGetName(heur), retcode);

      /* free buffer */
      SCIPfreeBufferArray(scip, &subvars);

      /* finally free subscipdata */
      SCIP_CALL( SCIPfree(&subscip) );

      return SCIP_OKAY;
   }
   else
   {
      SCIP_SOL** subsols;
      int nsubsols;

#ifdef SCIP_MORE_DEBUG
      /* print solving statistics of subproblem */
      SCIP_CALL( SCIPprintStatistics(subscip, NULL) );
#endif

      /* check, whether a solution was found */
      nsubsols = SCIPgetNSols(subscip);
      if ( nsubsols > 0 )
      {
         SCIP_SOL* newsol;

         subsols = SCIPgetSols(subscip);
         success = FALSE;

         probdata = SCIPgetProbData(scip);
         assert( probdata != NULL );
         network = probdata->network;

         for (i = 0; i < nsubsols && ! success; ++i)
         {
            SCIP_CALL( SCIPcreateOrigSol(scip, &newsol, heur) );
            for (j = 0; j < nvars; ++j)
            {
               SCIP_Real val;

               val = SCIPgetSolVal(subscip, subsols[i], subvars[j]);

               SCIP_CALL( SCIPsetSolVal(scip, newsol, vars[j], val) );
            }

#ifdef SCIP_DEBUG
            SCIP_CALL( SCIPcheckSolOrig(scip, newsol, &success, TRUE, FALSE) );
#else
            SCIP_CALL( SCIPcheckSolOrig(scip, newsol, &success, FALSE, FALSE) );
#endif
            if ( success )
            {
               SCIP_Real objval;
               objval = SCIPgetSolOrigObj(scip, newsol);
               SCIP_CALL( SCIPaddSolFree(scip, &newsol, &success) );
               SCIPdebugMsg(scip, "Sub-SCIP found a feasible solution of value %g (stored = %u).\n", objval, success);
               *result = SCIP_FOUNDSOL;
            }
            else
               SCIPdebugMsg(scip, "Sub-SCIP solution infeasible.\n");
         }
      }
      else
         SCIPdebugMsg(scip, "Sub-SCIP did not find feasible solution.\n");
   }

   /* free buffer */
   SCIPfreeBufferArray(scip, &subvars);

   /* finally free subscipdata */
   SCIP_CALL( SCIPfree(&subscip) );

#ifdef SCIP_DEBUG
   SCIPverbMessage(scip, SCIP_VERBLEVEL_NORMAL, NULL, "Finished solving heuristic <%s>.\n", SCIPheurGetName(heur));
#endif

   return SCIP_OKAY;
}


/*
 * primal heuristic specific interface methods
 */

/** creates the lpround primal heuristic and includes it in SCIP */
SCIP_RETCODE SCIPincludeHeurLPRound(
   SCIP*                 scip                /**< SCIP data structure */
   )
{
   SCIP_HEURDATA* heurdata;
   SCIP_HEUR* heur;

   /* create lpround primal heuristic data */
   SCIP_CALL( SCIPallocBlockMemory(scip, &heurdata) );

   /* include primal heuristic */
   SCIP_CALL( SCIPincludeHeurBasic(scip, &heur,
         HEUR_NAME, HEUR_DESC, HEUR_DISPCHAR, HEUR_PRIORITY, HEUR_FREQ, HEUR_FREQOFS,
         HEUR_MAXDEPTH, HEUR_TIMING, HEUR_USESSUBSCIP, heurExecLPRound, heurdata) );

   assert( heur != NULL );

   /* set non-NULL pointers to callback methods */
   SCIP_CALL( SCIPsetHeurCopy(scip, heur, heurCopyLPRound) );
   SCIP_CALL( SCIPsetHeurFree(scip, heur, heurFreeLPRound) );

   /* add lpround primal heuristic parameters */
   SCIP_CALL( SCIPaddLongintParam(scip, "heuristics/" HEUR_NAME "/nodelimit",
         "maximal number of nodes to use in sub-SCIP",
         &heurdata->nodelimit, TRUE, DEFAULT_NODELIMIT, -1LL, SCIP_LONGINT_MAX, NULL, NULL) );

   SCIP_CALL( SCIPaddBoolParam(scip, "heuristics/" HEUR_NAME "/transfersols",
         "Transfer solutions of main SCIP to sub-SCIP?",
         &heurdata->transfersols, TRUE, DEFAULT_TRANSFERSOLS, NULL, NULL) );

   return SCIP_OKAY;
}
